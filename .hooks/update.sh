...
if grep -q " git@github.com:" .git/config; then
  ...
  # ssh-agent bash -c "ssh-add ~/.ssh/sb_id_rsa_nopass && git fetch --all && git reset --hard origin/master && git submodule update && yarn i --production && yarn add youtube-dl"; kill $SSH_AGENT_PID
  ssh-agent bash -c "ssh-add ~/.ssh/sb_id_rsa_nopass && git fetch --all && git reset --hard origin/master && git submodule update && yarn install --production"; kill $SSH_AGENT_PID
else
