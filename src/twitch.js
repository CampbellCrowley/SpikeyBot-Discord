  ...
  _commandTest() {
    const parsed = {
      'data': [{
        ...
        'id': '35822878240',
        'language': 'other',
        'started_at': '2019-09-29T22:15:48Z',
        'tag_ids': null,
        'thumbnail_url':
            'https://static-cdn.jtvnw.net/previews-ttv/live_user_spikeyrobot2' +
            '-{width}x{height}.jpg',
        'title': 'spikeyrobot\'s Test Channel(Testing a new setup)',
        'type': 'live',
        'user_id': '67729681',
        'user_name': 'spikeyrobot2',
        'viewer_count': 0,
      }],
    };
    const data = parsed.data[0];
    ...
    global.sqlCon.query(toSend, (err, rows) => {
      if (err) {
        this.error('Failed to fetch TwitchDiscord database info.');
        console.error(err);
        return;
      }
